package generativelayers.tests;

import generativelayers.body.*;
import generativelayers.kernel.*;
...
final class KernelTest {
    ...
    void metricsTrackInvocationOutcomes() {
        ...
        assertTrue(metrics.stream().anyMatch(m -> m.contains("generativelayers.invoke.total")), "Must track total invocations");
        assertTrue(metrics.stream().anyMatch(m -> m.contains("generativelayers.invoke.success")), "Must track successful invocations");
        assertTrue(metrics.stream().anyMatch(m -> m.contains("generativelayers.invoke.denied")), "Must track denied invocations");
    }
}
