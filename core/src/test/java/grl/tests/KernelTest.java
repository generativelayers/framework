...
final class KernelTest {
    ...
    void metricsTrackInvocationOutcomes() {
        ...
        assertTrue(metrics.stream().anyMatch(m -> m.contains("grl.invoke.total")), "Must track total invocations");
        assertTrue(metrics.stream().anyMatch(m -> m.contains("grl.invoke.success")), "Must track successful invocations");
        assertTrue(metrics.stream().anyMatch(m -> m.contains("grl.invoke.denied")), "Must track denied invocations");
    }
}
