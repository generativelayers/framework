package grl.provider.openai;

import grl.kernel.Blob;
import grl.kernel.KernelPorts;
import grl.kernel.ProviderOutput;
import grl.kernel.ResourceRequest;
 ...
 *   <li>{@code apiKeyEnv} — env var holding the API key (default: GRL_OPENAI_API_KEY)</li>
 * </ul>
 */
public final class OpenAiCompatibleProvider implements KernelPorts.GenerativeProvider {
    ...
    /**
     * Factory method — called by {@link grl.provider.ProviderRegistry} via reflection.
     */
    ...
    public static OpenAiCompatibleProvider fromEnvironment() {
        return new OpenAiCompatibleProvider(
                new ProviderConfig.Builder()
                        .set("provider", "openai")
                        .set("apiKeyEnv", "GRL_OPENAI_API_KEY")
                        .build()
        );
    }
